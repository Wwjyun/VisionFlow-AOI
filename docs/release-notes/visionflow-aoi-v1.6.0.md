...
- 影像仍在 CPU 解碼；GPU 模式下整張圖只做一次 resident upload，之後 Template Anchor Grid 定位、ROI 產生與前處理直接在 device 上執行，不再逐 tile 複製 CPU 影像。
...
- 新增 bit-exact GPU float32 median（cub radix sort）、支援 sigma 的 float32 Gaussian、融合 residual mask export、GPU Anchor Grid 定位，以及與 OpenCV 逐點等價的 `RETR_LIST` contour 追蹤（目前未接入 Detector，因稀疏影像仍慢於 CPU）。
