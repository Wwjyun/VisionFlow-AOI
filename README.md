...
- `202-CS-SN-1`、`203-AS-SN-1`、`401-AS-SN-1`、`401-CS-AP-1`、`401-CS-AP-2`、`401-CS-SN-1`、`503-CS-SN-1`、`505-AS-SN-1`、`506-CS-SN-1`、`900-CS-AP-1` 十個傳統電腦視覺 Detector，以及 ONNX Runtime `yolox` Detector。
- 單張檢測、批次資料夾檢測及新檔案監控。
...
最新發行版為 [v1.6.2](https://github.com/wjcudalearning/VisionFlow/releases/tag/v1.6.2)（CUDA-enabled Windows x64，`sm_86`）。本版把 `202-CS-SN-1` 的 morphology、connected components、面積／邊界過濾與 ring CNR 留在 GPU，只下載候選統計；BMP 以平行分段讀取並在 GPU resident 路徑保留檔案列序，省去 CPU 全圖翻轉。RTX 3090 正式尺寸合成圖（16384×13000、6 個高 12000×寬 2000 ROI）端到端 CPU 5453.3 ms、GPU **397.7 ms（median，13.71×）**，3/3 輪判定欄位完全一致，詳見〈[新 GPU mode 效能](#新-gpu-mode-效能)〉與 [`gpu/README.md`](gpu/README.md)。
...
|   `-- camera_monitor_processor.py # 相機直連監控（有界 frame 佇列與依序檢測）
|-- detectors/
...
|   |-- detector_401_cs_sn_1.py     # 401-CS-SN-1 自適應輪廓檢測
|   |-- detector_503_cs_sn_1.py     # 503-CS-SN-1 固定二值化多邊形檢測
...
CPU 切圖預設使用 `auto`：先檢視前 16 張 ROI，至少 4 張且總裁切量達 8 MiB 時使用最多 4 個 CPU workers，其餘維持序列，避免小 Tile 被排程成本拖慢。配方 `performance.crop_workers: 1/2/4/...` 或環境變數 `AOI_CROP_WORKERS` 可覆寫（配方優先，worker 數不超過可用 CPU 核心數）；獨立使用 `Tiler.from_config` 或 `create_tiler` 時也可在 `tile.crop_workers` 指定。一般 grid、先對整張圖做 Template Anchor Grid Pattern Match 再按 rows/cols/ROI/gap 切圖、contour 與 pattern_match 模式皆適用；定位與座標計算維持原順序，只有獨立 ROI 的像素複製進入工作池，輸出順序不變。CUDA crop 或 resident image 不進入 CPU 平行裁切。此門檻取自本機合成尺寸測試，量產最佳值仍應用實際影像量測 `tiling` 與端到端時間。
...
| `401-CS-SN-1` | 自適應輪廓檢測 | 不適用 |
| `503-CS-SN-1` | 固定二值化多邊形檢測 | 不適用 |
...
| `401-CS-SN-1` | 四邊內縮、最小／最大面積 | 邊緣屏蔽、Adaptive Mean block／C／反相／最大值及輪廓模式 |
| `503-CS-SN-1` | 中心屏蔽半寬／半高、四邊內縮、最小／最大面積 | 中心／邊緣屏蔽開關、中心定位、固定 threshold／反相／最大值、輪廓模式及多邊形近似設定 |
...
- 檔案：`detectors/detector_401_cs_sn_1.py`
- 預設流程：Gray → Adaptive Mean 一般二值化 → 四邊排除屏蔽 → `RETR_LIST` contours；不使用 Gaussian blur 或形態學，抓到任一符合面積條件的輪廓即 NG，沒有輪廓即 PASS。
- 調參值：Adaptive block 保存為 `156`、C 為 `-56`；依調參工具與 OpenCV 契約，偶數 block 在執行時自動轉成下一個奇數，因此有效 block 為 `157`。metadata 同時保存設定值與有效值。
- 四邊屏蔽預設啟用，共同／左／右／上／下內縮預設皆為 `0`，可由 Recipe Designer 分別調整；各邊實際值為共同內縮與個別值兩者的較大值。
- 面積：`min_area`／`max_area` 預設皆為 `0`，代表不限制；只排除面積為零的輪廓。
- 缺陷類型：`401_cs_sn_1_contour_ng`

### `503-CS-SN-1`：固定二值化多邊形檢測

- 檔案：`detectors/detector_503_cs_sn_1.py`
...
- 中心與四邊屏蔽皆預設啟用；中心預設使用影像中心，半寬／半高與各邊內縮預設皆為 `0`。尺寸為工程外參；屏蔽開關、中心定位與影像演算法設定為管理內參。
- 缺陷類型：`503_cs_sn_1_polygon_ng`
...
- **監控模式**：監控資料夾並逐張處理新加入且已穩定的影像；影像來源也可切換為「相機直連」，逐張檢測相機觸發完成的影像。
...
2. 載入配方。
3. 在設定抽屜確認輸出項目。
4. 使用 GPU 的配方可先按「GPU 預熱」（選用），讓第一張檢測不承擔 CUDA 初始化。
5. 執行檢測。
6. 查看 PASS／NG、Tile、NG 及缺陷數量與耗時。
7. 檢查 Overlay、缺陷表格與輸出檔案。
...
監控來源（監控資料夾／相機直連）只能在 Engineer 或 Admin 模式、且監控未執行時切換，選擇會在下次啟動時恢復。選擇「相機直連」時畫面改顯示相機連線狀態與觸發模式：

- 相機必須已在 CCD 控制以**外部觸發或軟體觸發**連線，「啟動」才可按；連續取像的預覽影像不會被檢測，畫面會說明原因。
- 啟動後，每張觸發完成的影像直接在記憶體中送檢，不先存檔；判定與把同一張影像存成 BMP 再檢測完全相同。結果與資料夾監控一樣顯示在表格與散佈圖，輸出寫到 `outputs\monitor\<時間>_camera\`，影像名稱為 `camera_<日期時間>_<序號>`，JSON 另記錄 frame 序號、擷取時間與觸發模式。
- 檢測佇列最多暫存 4 張影像；檢測跟不上時，超出的影像會以 ERROR「檢測佇列已滿，此影像未檢測」列在表格，不會默默略過。
- 按「停止」後不再接收新影像，已進佇列的影像仍會檢測完才結束。
- 相機直連不另存原圖；需要原圖時，請在 Recipe 相機設定開啟自動存圖。
...
- 觸發自動化只依「目前連線時實際寫入相機」的觸發設定動作，未重連前修改的觸發設定不會生效：
  - **外部觸發**：外部觸發單張＋「外部觸發時自動寫入已存 Compare 值」開啟時，每次觸發會把米輪區塊保存的 Compare 值寫入卡片，再勾「同時寫入已存 Encoder 值」會一併寫回 Encoder 原點。米輪未連線時只在狀態列提示。
  - **軟體觸發**：「開始軟體觸發」不做連續取像，而是每 50 ms 讀 Encoder。Encoder 低於已存 Compare 值時寫入 Compare 並擷取一張（每條線仍由米輪脈衝觸發）；要等 Encoder 回到 Compare 以上才會再擷取下一張。需先連線米輪；「停止」只停止監控，擷取中的影像會繼續收完。米輪讀值失敗時自動停止監控。
  - **自動存圖**：外部觸發單張模式下，收到觸發後完成的那一張會保存；軟體觸發模式下每張完成的影像都會保存，同一張不會存兩次。
...
- 202-CS-SN-1 有 resident ROI 且未開啟 debug 影像時，優先使用 `vf_cnr_mask_u8_roi`：直接讀取已上傳的原圖 ROI，在 GPU 完成 BGR→gray、float32 Gaussian、residual、median／MAD、門檻與候選遮罩，只下載遮罩與三個診斷純量，metadata 標示為 `cuda_resident_fused`。它與既有 GPU chain 逐位元相同（15/15），候選遮罩與 CPU 相同（15/15）；舊 DLL、缺少 export、呼叫失敗或 debug 模式會改走下述既有路徑。
- 202-CS-SN-1 有 resident ROI 且 DLL 具 `vf_cnr_candidates_u8_roi` 時（v1.6.2 起），morphology、排除區、connected components、面積／邊界過濾與 ring CNR 統計也全部留在 GPU，只下載每個候選一筆 40 bytes 的紀錄；背景 mean/std 依 NumPy float32 pairwise 順序逐位元重現，70/70 等價案例與既有路徑逐欄相同，metadata 以 `component_backend`（`opencv_cpu`／`cuda_resident`）標示。ring 統計的像素收集與 pairwise 葉節點全部平行計算，256×256 起即快於既有路徑（512×512 為 2.59×、12000×2000 為 10.43×），因此不設 ROI 尺寸下限；ring 背景不足、參數不支援或任何錯誤會整段改走既有路徑。
...
**v1.6.2：CCL＋ring CNR 移到 GPU、BMP 直讀並省略 CPU 全圖翻轉。** 同一正式尺寸基準端到端 CPU 5453.3／5684.8 ms、GPU **397.7／418.9 ms（median／P95，13.71×）**；Detector 合計 CPU 5147.0 ms、GPU 86.3 ms（59.66×），Tiling 67.0→2.45 ms。每輪 GPU 傳輸只有整圖上傳 638,976,000 bytes、候選與 anchor 結果下載 22,340 bytes，共 8 次 native calls；3/3 輪判定欄位完全相同。24-bit BMP 在 GPU resident 模式保留 bottom-up 檔案列序，一次 H2D 後於 device 翻列；舊 DLL 缺少 optional `vf_context_upload_u8_file_order` 時安全回退原路徑。詳見 [`gpu/README.md`](gpu/README.md)〈BMP file-order 直讀與 resident upload〉。
...
.\env\Scripts\python.exe -m tools.export_tile_defect_distribution --input .\outputs\<run>\csv\summary.csv
```
...
| Tile 缺陷分布 HTML 報表 | `.\build_tile_defect_distribution_exporter.ps1` | `dist\Tile-Defect-Distribution-Exporter\export_tile_defect_distribution.exe` |

五支 EXE 都可直接雙擊開啟 GUI，也保留命令列模式；`--help` 顯示參數、`--version` 顯示工具版本、`--smoke-test` 可供非互動打包驗證。它們都是 CPU-only 後處理／切圖工具，不執行 AOI Detector、不收錄 CUDA DLL，也不需要另外安裝 Python。
