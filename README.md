...
- 202-CS-SN-1 的 ROI 面積 ≥ 1024×1024 且 DLL 具 `vf_cnr_candidates_u8_roi` 時（`main`，v1.6.1 之後），morphology、排除區、connected components、面積／邊界過濾與 ring CNR 統計也全部留在 GPU，只下載每個候選一筆 40 bytes 的紀錄；背景 mean/std 依 NumPy float32 pairwise 順序逐位元重現，71/71 等價案例與既有路徑逐欄相同，metadata 以 `component_backend`（`opencv_cpu`／`cuda_resident`）標示。較小的 ROI 因 ring window 在單一 GPU thread 內循序處理而較慢（512×512 為 0.47×），維持既有路徑；ring 背景不足、參數不支援或任何錯誤也會整段改走既有路徑。
...
**`main`（v1.6.1 之後）：CCL＋ring CNR 移到 GPU。** 同一正式尺寸基準：端到端 CPU 6725.5 ms、GPU **1467.7 ms（4.58×）**，GPU 端比 v1.6.1 的 1957.5 ms 快 25%；Detector 合計 CPU 5648.1 ms、GPU 334.4 ms（16.9×）。每輪 GPU 傳輸只剩整圖上傳 639 MB 與 22 KB 的候選紀錄（v1.6.1 需下載 288 MB 的 gray 與 mask），native call 由 14 次降為 8 次；3/3 輪判定欄位完全相同。GPU 模式剩下最大的一段是影像解碼（約 829 ms，57%）。詳見 [`gpu/README.md`](gpu/README.md)〈CCL＋ring CNR 留在 device〉。
