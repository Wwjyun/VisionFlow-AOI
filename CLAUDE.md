...
- The working tree usually contains untracked user artifacts, now grouped under `簡報/` (投影片＋`.inspect.ndjson` 側錄檔) and `架構圖/` (含 `runtime-architecture/`、`charts/`、`_backup_20260911/`), plus `cuda_course/`, `interview_prep/`, `xx_ccd/` (C# camera app, reference for Todo P11 only), `docs/reports/*.md` drafts, and `.codex_ppt_build/`. Never stage or delete them; the full map and the move rules are in `ARTIFACTS.md`.
