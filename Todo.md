...
- [x] 2026-08-29：將主帳號 repository 重新命名為 `Wwjyun/VisionFlow-AOI`，以 fast-forward 同步另一帳號延續開發的 56 個 commits 與 8 個新增 tags；再搬移 `v1.2.0` 至 `v1.5.1`、Traditional CV Tuning Tool 與 Utility Tools 共 8 個 GitHub Releases、9 個公開附件（1,173,073,599 bytes），逐檔重新下載核對 size／SHA-256，確認 16 個 Releases、0 草稿、`v1.5.1` 為 Latest。同步更新 repository 與本機 `aoi-release` skill 的發布目標；完整 308 tests、compileall、CUDA source preflight、CLI PASS smoke、GUI offscreen smoke及 `git diff --check` 均通過。
