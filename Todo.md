...
- [ ] **2026-09-15 使用者排定的新 GPU mode 下一步優先順序**（依序執行；P0 anchor 接線與 split 誤報已完成）：
  ...
  4. 初始化預熱：預熱按鈕已完成（第一張 2075.5 → 1859.2 ms）；監控模式已自動預熱、批量模式開始前檢查 CUDA context（2026-09-17）；session 重建條件與 GUI／批量／監控共用 session 已完成（2026-09-17，見完成紀錄）；剩「載入 GPU Recipe 與影像後自動背景預熱」待使用者決定（見 P6）。
...
- [ ] **載入 GPU Recipe 與影像後自動背景預熱（待使用者決定）**：OP 模式看不到預熱按鈕。session 共用後，任何一次單張、批量、監控或預熱都會讓後續執行保持 warm，只剩開程式或換 GPU 設定後的第一次冷啟動（RTX 3090 正式尺寸第一張約多 120～150 ms）。自動預熱需要以目前影像完整試跑一次並在期間鎖住檢測／換圖／換 Recipe，每次換圖或載入 Recipe 都可能觸發，屬操作流程改變，需使用者決定觸發時機（例如只在 OP 模式載入 Recipe 後、只建立 context 不試跑，或維持手動）。
