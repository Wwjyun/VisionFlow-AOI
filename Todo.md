...
- [x] 2026-09-14：讓 `auto` 模式的 CPU 路線判定同時抑制整圖 resident 上傳。`PlanCrossoverPolicy` 新增 `prefer_cpu_key(key)`，回傳（是否走 CPU、用於回報的 decision key），取代原本分開查詢的 `prefers_cpu`：目前 key 量到 CPU 較快時成立，host-input 呼叫另可沿用同一 plan／shape 的 resident／device ROI 決定（host-input CUDA 另付 H2D，所以對 resident 路徑量到的CPU 決定在略過 resident 影像後仍成立；反向不成立）。`mark_resident_upload_unneeded`／`resident_upload_unneeded` 以 bounded LRU 保存 `(effective_recipe_sha256, image.shape)` 鍵。`core/pipeline.py` 在 `detectors_total` 之後、序列與平行路徑共用的位置檢查：該次執行的每個原生 CUDA Detector 都必須 `cpu_crossover_covers(policy)`，即它實際跑過的每個 `(plan.signature, image.shape, device_roi)` key 仍解析為 CPU 且該次沒有 CUDA 路線；不同門檻／tile／影像 shape 各自保有校準金鑰並維持上傳，未量測的 shape 不會沿用別人的略過決定。Pipeline 下次讀到標記時 `resident_image` 維持 None，完全不發生像素 H2D、tiling 改走 CPU tile、Detector 以 `device_roi=None` 執行同一份 plan 的 CPU 路線；`BaseDetector._execute_cuda_or_crossover_cpu` 以 `_crossover_plan_keys` 記錄這些 key 供 `cpu_crossover_covers` 重查，`preprocess_route_counts` 與 `cpu_crossover_only` 語意不變；`core/pipeline_stages.py` 在 `execution.gpu.resident_image` 既有 `active`、`generation`、`shape`、`device_memory_before_upload` 旁新增 `skipped_by_crossover`（bool）。strict `gpu.mode: cuda` 不建立 crossover policy、永不略過、禁止隱性 CPU fallback，`gpu.mode: cpu` 不載入 CUDA，只有 `auto`＋`fallback_to_cpu` 可走此路徑；plan 的 CPU 路線與 CUDA 逐像素相同，檢測結果不變。此為 `### 全流程 GPU 化` 的局部進展：contour／幾何／CNR 候選抽取與 Template Anchor Grid 定位仍在 CPU 執行，仍列於該節待辦，未因此變更。測試：`tests/test_gpu_crossover.py` 13 tests OK，含真 Detector401、真 `BaseDetector` 路線計數與真 `GpuExecutionSession` 的端到端測試（同一 Recipe／shape 連續 4 張，第 4 張起 `skipped_by_crossover=true` 且不發生 H2D，第 3、4 張的 final_result／summary／tile 結果完全相同）、CUDA 較快時仍每張上傳的回歸測試，以及 shape 變更（512×480）不沿用 512×512 略過決定的回歸測試。未修改 CUDA source／header／ABI／DLL，因此未執行 RTX 3090 硬體驗收。
