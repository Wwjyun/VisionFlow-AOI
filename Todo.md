...
- [ ] **2026-09-15 使用者排定的新 GPU mode 下一步優先順序**（依序執行；P0 anchor 接線與 split 誤報已完成）：
  1. CCL＋ring CNR 留在 device：可省約 279 ms CPU 時間，並把 144 MB gray D2H＋144 MB mask D2H 歸零（每輪只剩 639 MB 整圖上傳），是剩下最大的單一缺口；啟用條件見 `gpu/README.md`〈GPU connected components 實驗〉。
...
- [ ] **`device_host_split` 與 `execution.gpu.metrics` 在共用 session 下是累計值**（2026-09-15 修 anchor 時發現）：`InspectionResultAssembler._device_call_counts` 讀 `runtime.performance_stats()`，而 GUI 單張、batch 與 monitor 共用同一個 runtime，呼叫次數與傳輸量從 session 建立起一路累加（正式 benchmark JSON 的 `gpu_metrics` 即為 warm-up＋3 輪合計）。因此某一輪沒有呼叫 export，只要先前任一輪呼叫過，`automatic_cnr_mask`／`candidate_extraction` 等仍會回報 `device`。改為每輪開始記錄快照、結果回報本輪差值（保留累計值另列），並補「第二輪未呼叫 export 時回報 cpu」測試。
