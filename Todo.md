...
- [ ] 202-CS-SN-1 自動 CNR 主體 GPU 化：新增共用 typed operator 與 CUDA 實作，涵蓋 float 大核 Gaussian 背景（含 sigma 自動規則）、residual、MAD（與 NumPy median 完全相同的中位數）、`max(8, 3×sigma)` 門檻 mask、形態學、8-connectivity connected components（標籤順序與 `cv2.connectedComponentsWithStats` 的 stats 完全一致）、局部背景 ring CNR 統計，只下載候選統計。**2026-09-15 進度**：Gaussian 背景、residual、兩個 median／MAD、門檻、候選遮罩**皆已 GPU 化並接入產線**（`vf_gaussian_blur_f32`、`vf_cnr_mask_f32`），產線形狀端到端 **1.26×**、decision-bearing 欄位完全相同。**剩下兩項**：（1）**8-connectivity connected components 的標籤順序**——唯一真正的阻擋，已定位為「忠實實作 OpenCV 的 Bolelli 2×2 區塊掃描」，可完成但成本高（見本節缺口段落）；（2）**形態學與 ring CNR 統計仍在 CPU**（`connected_components_and_cnr` 約 52～57 ms、ring CNR 的背景 mean/std 以 NumPy 計算）；依既有量測其收益遠小於已完成的步驟，優先序低於 CCL。**已放棄的方向**：輪廓抽取（`vf_find_contours_u8`）在每一個量測形狀都慢於 cv2，不接入產線也不訂啟用界線。（2026-09-15 **進行中（已委派）**：把 residual 留在 device 的融合路徑。現況是 `_automatic_cnr_mask` 的 Gaussian 與兩次 median 都已在 device 上，但每次 `vf_median_f32` 都重新上傳整個 operand——2000×12000 ROI 為 2×96 MB H2D 加上 `np.abs(residual - median)` 這個 host 暫存陣列，量測顯示 `threshold_and_mask` 仍佔 94.0 ms、`Gaussian+residual` 佔 59.0 ms（`outputs_validation/cnr_profile/mask_stage_split.json`）。已委派新增一個 **additive optional ABI v1 export `vf_cnr_mask_f32`**：輸入整張 device-resident 的 image 與 background，在 device 上一次完成 residual、**與 `np.median(float32)` 位元相同**的 median、`mad = median(|residual − median|)`、`threshold = max(floor, mult × max(scale×mad, abs_floor))`、以及嚴格 `>` 比較的候選遮罩，只回傳 3 個 float 與一張 uint8 遮罩（不下載 residual）。驗收要求：median／mad／mask 必須**逐位元相同**（門檻值必須為 0 差異或明確說明為何做不到）、決定性、不改動輸入、非法參數明確拒絕、以及 2000×12000 的 device event 計時與節省的上傳位元組量化。**不得先鬆動驗收門檻讓它通過**；若無法逐位元，要求如實回報失敗案例。子代理回報後由主 session 自行重跑並接線（`detectors/` 由主 session 擁有）。）（2026-09-14 第十一次嘗試：改攻契約列為收益最大的 202-CS-SN-1 CNR，先做階段量測決定 GPU 化順序。以 2000×4000（產生器合成、含 24 個嵌入缺陷）ROI 量測 CPU 實作（`outputs_validation/cnr_profile/cnr_profile.txt`）：background kernel=51、sigma=0，`cv2.GaussianBlur`(float32) 15.4 ms、`np.median(residual)` 68.7 ms、`np.median(|residual-median|)`（MAD）159.7 ms、完整 `detector.detect()` 305.2 ms。**結論：中位數運算佔約 75%，是首要 GPU 化目標；Gaussian 只佔 5%，不是瓶頸**（此點與先前假設不同）。接著做直方圖中位數的精度研究（`outputs_validation/cnr_profile/histogram_median_precision.txt`）：對零中心 float32 殘差，4096 bins 絕對誤差 9.8e-3、16384 bins 8.9e-4、65536 bins 1.5e-4、262144 bins 3.8e-5；MAD 在 65536 bins 時誤差 1.0e-4。因此若要與 NumPy 等價，需 65536 bins 以上；但 65536 個 int32 為 256 KB，超過單 block 共享記憶體上限，只能走 global histogram 或排序式精確中位數（bitonic/thrust），`noise_sigma_floor` 預設 1e-6 使精度要求偏嚴，採直方圖前必須先定義並記錄容差契約。此項尚未實作任何 CUDA；下一步建議以「GPU 排序取精確中位數」實作 `vf_median_f32`（與 `np.median` 逐位相同、無需容差），或先與使用者確認可接受的 median 容差再走直方圖。）
...
- [x] 2026-09-15：完成**產線真實幾何**的端到端 CPU/GPU 驗證，並修正一個會讓新 export 靜默失效的整合缺陷。`tools/benchmark_pipeline_production.py` 新增 `--profile production`（六個 **12000h×2000w** ROI、6000×24000 canvas），保留 `--profile quick`（六個 2000×2000）。**主 session 親自量測**（單次，RTX 3090）：CPU 端到端 **7141.0 ms** → CUDA **4067.0 ms**，**1.76×**；`202-CS-SN-1.automatic_cnr_mask` **4850.8 → 1682.7 ms（2.88×）**、`detectors_total` **5250.8 → 2083.8 ms（2.52×）**；PASS/NG 6/6、缺陷數 6 tiles／**558 defects** 完全相同，**decision-bearing 欄位完全相同**，而且**這個真實幾何場景上 residual 衍生的四個診斷值完全沒有漂移**（`no drift on this image`）——先前的漂移只在合成的 2000×2000 場景出現。未被 GPU 覆蓋的固定成本也如實記錄：`image_load` 1682→1688 ms（兩邊相同，占 CUDA 總時間 41%）、`recipe_setup` 103→204 ms、`initialization` 0.1→70 ms。**整合缺陷（子代理發現、我確認並保留修正）**：我的呼叫端寫成 dict（`device["mask"]`），子代理第一版 bridge 回傳 4-tuple，`TypeError` 會被呼叫端的 `except Exception: pass` 吞掉，導致新 export **從未被實際使用**——這正是「寬鬆 except 讓接線靜默失效」的典型陷阱。修正後 bridge 回傳 dict，我用真 DLL 實測確認：`background_backend=cuda_f32`、`residual_backend=cuda_f32`、`vf_cnr_mask_f32: 1`、**`vf_median_f32: None`**（融合 export 真的取代了兩次 median）。此缺陷也解釋了先前的量測不一致：commit `87de895` 當下的 bridge 形式未定，該次記錄的 1.26× 可能來自 NumPy 回退路徑；本輪的 1.76× 是在**確認融合路徑生效**後量的。全套 **425 tests OK**、compileall exit 0、`gpu/preflight_cuda_build.py` exit 0。證據：`outputs_validation/cnr_profile/pipeline_production_real.json`、`pipeline_production_quick.json`。
